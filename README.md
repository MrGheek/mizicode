...
**Spin up on-demand, GPU-free AI coding workspaces and orchestrate autonomous coding agents with a built-in IDE, memory system, collaborative multi-agent lanes, and an MCP server.**

MIZI is a full-stack, self-hostable platform that provisions a lightweight
**CPU-only** workspace (Theia IDE) on demand and routes all model inference to
**hosted NVIDIA NIM** (or any OpenAI-compatible endpoint) through an
in-container proxy — no GPU machines, no local model downloads, no vendor
lock-in.
...
|---|---|
| GPUs are expensive and scarce | **CPU-only workspaces** — inference runs on hosted NVIDIA NIM / any OpenAI-compatible API, so you never provision a GPU |
...
| Opaque agent cost | **Universal spend ledger** — per-session token accounting, tripwires, token modes, and cost-per-work-order factory telemetry |
...
1. **API server** (`artifacts/api-server`) — Express 5 backend that orchestrates
   session lifecycle, workspace provisioning (Fly Machines API), memory, skills,
   plan board, lane coordination, the Code Factory, ambient agent, and an MCP
   server.
