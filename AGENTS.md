...
| 15 | `mizi-git-lanes` | frontend | services/lane-branch.ts, services/github-pr.ts | SCM commands: session/lane branch, handoff to PR, push | P1 |
| 16 | `mizi-claw-runner` | both | routes/sessions-swarm.ts | Swarm job start/stop/stream from IDE | P1 |
...
| 20 | `mizi-lane-coordinator` | frontend | routes/coordination.ts, services/lane-policy.ts | Status bar (claim, blast radius, handoff) | P2 |
| 21 | `mizi-vllm-manager` | backend | — (calls /vllm/*; backend route not implemented) | Local vLLM process control: start/stop/config | P2 |
