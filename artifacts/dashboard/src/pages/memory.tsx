...
import { Brain, Search, X, Clock, ChevronDown, ChevronRight } from "lucide-react";
...
function useGlobalSearch(query: string) {
  return useQuery<MemorySearchResult>({
    queryKey: ["mem-global-search", query],
    enabled: query.trim().length > 1,
    queryFn: async () => {
      const res = await fetch(`${BASE_URL}api/memory/search?q=${encodeURIComponent(query)}`);
...
export default function MemoryPage() {
  ...
  const sessionsWithoutSummaries = (sessions || []).filter(s => !s.summary);
