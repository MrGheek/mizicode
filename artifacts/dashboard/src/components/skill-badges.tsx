...
export function SkillClassBadge({ skillClass }: { skillClass: string }) {
  const map: Record<string, { label: string; cls: string }> = {
    ...
    research:   { label: "Research",   cls: "bg-sky-500/20 text-sky-400 border-sky-500/30" },
  };
...
export function TrustBadge({ trustTier }: { trustTier: string }) {
  const map: Record<string, { label: string; cls: string }> = {
    mizi_native:  { label: "MIZI Native",  cls: "bg-emerald-500/20 text-emerald-400 border-emerald-500/30" },
