...
export function buildOnStartScript(profileConfig: {
  ...
  const nimLines = profileConfig.nimModelId
    ? [
        ...
        // ── Inline nim-proxy.py patch (double-v1 URL fix) ──────────────────────
        // Overwrites /opt/nim-proxy.py before /opt/onstart.sh starts the proxy.
        // This ensures the fix is active on every new NIM session regardless of
        // which image version is running in the workspace machine.
        `# ── Patch nim-proxy.py (double-v1 URL fix, inline before onstart.sh) ──`,
        `cat > /opt/nim-proxy.py << 'NIMPROXY_EOF'`,
        `#!/usr/bin/env python3`,
        `"""`,
        ...
        `NIMPROXY_EOF`,
        `echo "[nim-proxy-patch] nim-proxy.py patched OK (double-v1 URL fix)" >> /var/log/onstart.log`,
      ].filter(Boolean).join("\n")
    : "";
