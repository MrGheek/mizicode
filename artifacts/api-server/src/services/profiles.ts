...
const DEFAULT_PROFILES: InsertGpuProfile[] = [
  ...
  // H200: 8× H200 — full context, recommended
  {
    ...
    llamaCtxSize: 131072,
    llamaBatchSize: 1024,
    // Chunked prefill + priority scheduling (vLLM ≥ 0.7.0)
    // H200 provides additional VRAM headroom vs H100 Ultra; swarm is viable but
    // limited by the 40B active parameter footprint.
    llamaExtraArgs: `--kv-cache-dtype fp8 --enable-expert-parallel --tool-call-parser glm47 --reasoning-parser glm45 --enable-auto-tool-choice --speculative-config.method mtp --speculative-config.num_speculative_tokens 3 ${CHUNKED_PREFILL_PRO}`,
