  ...
  },
  {
    ...
    instructions: {
      system: [
        ...
        "If a command is blocked by the shield, do not attempt to bypass the safety check — report the block and suggest an alternative approach.",
        "Shielded output is saved to /workspace/.floatr/artifacts/ and is available for the duration of the session.",
        "Run `floatr_stats` to see how many bytes of context the shield has saved this session.",
        "Run `floatr_doctor` to check shield health (journal status, artifact cap, stale files).",
      ],
    },
    install: { type: "virtual", outputs: ["system_prompt_fragment"] },
  ...
  },
  {
    ...
    instructions: {
      system: [
        ...
        "Before any compaction or major context reset, write a snapshot via the Claw Runner: POST /floatr/snapshot with your current working state (activeTask, planCheckpoint, activeFiles, unresolvedErrors, bundleSlug, tokenMode).",
        "Keep the snapshot concise: one sentence per active task, a bulleted plan checkpoint, and a list of at most 5 active file paths.",
        "After restoring from a snapshot, validate its fields against the current filesystem state before using them — the snapshot may be stale if files changed between sessions.",
        "Use the journal for debugging continuity issues: GET /floatr/stats or run `floatr_stats` to see restore counts and event history.",
