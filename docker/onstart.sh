...
(
    ...
    if [ -n "${NIM_MODEL_ID:-}" ]; then
        ...
        # Apply inline patch: overwrite /opt/nim-proxy.py with the current fixed version.
        # This ensures the double-v1 URL bug fix is always active, even on older images
        # that predate the fix being baked into the Dockerfile.
        log "Patching nim-proxy.py (double-v1 URL fix)..."
...
NIMPROXY_EOF
        log "nim-proxy.py patched OK (double-v1 URL fix applied)"
